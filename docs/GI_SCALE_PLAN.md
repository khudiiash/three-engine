...
**6.25 (NOT SHIPPED), 2026-08-29, branch rc5-faces `8460bc5`→`41a6c85` — the emitter penumbra must follow the lamp's size.** Report: the ×2 lamp measured the same 9 px ramp as the ×1. Mechanism (6.23 lane, confirmed): rcDirect's half-width was `lampHalf·t_occ/max(d−t_occ, lampHalf)` and the floor made W = t_occ for every blocker within a lamp-half of the lamp — both Cornell lamps — so the size cancelled. The right law (area light, planar blocker, first order): **W = L·(d_r − d_b)/d_b = L·t_occ/d_b**, L = the OBB's projected extent perpendicular to the seat ray, d_b/d_r = lamp SURFACE → blocker/receiver measured from the seat ray's exact OBB entry (6.14), ε only, no floor. Implemented (`8460bc5`, both arms, second coarse filter round R_MAX2 = 64 + stride-6 blocker search; `probe:gi2-cornell LAMP_SCALE=<k>` scales the lamp for truth AND ours) — **and it erased the wall's dip at both sizes** (×1 median |log| 0.181 → 0.201; ×2 ours flat ≈ 6.2 where truth ramps 10.9 → 5.3): a blurred BINARY mask cannot keep an umbra once W is comparable to the shadow (a 1 m lamp 1-1.7 m from the block gives W ≈ 1-2 m). 6.25b (`1c2275e`): the penumbra structure from GEOMETRY — 4 shadow rays per texel to the 2×2 quadrant centres of the emitter's near face (the centre ray's slab-exit axis in the emitter frame; fixed points, nothing stochastic), vis = mean (5 levels, a fully blocked texel stays 0), blur width W/4, `d_b ≥ 0.05·d_r` guard for the mover arm. **Receipt that stops it: the lit back wall reads 0.28-0.34 (6.25 control 0.925 with the same test script)**, and the regression survives every isolation: quadrant spread 0 (all four rays to the face centre), spread −1 (all four rays = the 6.25 centre ray, byte-identical geometry and `vis = 1 − h`), `nearestT` instead of `anyHit`, and 6.25d's ONE wgslFn `gi2BvhQuadVis` (`41a6c85`, the four rays + centre nearest-t over one binding set). The uncaptured-error hook (`scripts/lib/webgpuErrorLog.mjs`) reports **no GPU error in any run** — the pipeline builds; the exact arm is LIVE (74 tris). Clue: **the ×2 lamp once PROMOTED to a mover (excluded from the static tree) reads lit 0.999** — the rays are being stopped by the lamp's own triangles in the static soup, although the hand near face (B = identity, axis z, sign −1, F = (−0.015, 2.828, 1.013)) says a wall→face segment cannot enter the box and each ray ends max(dq·10⁻³, 2 mm) short. The rcDirect debug texture (mode 1/2) reads (vis,vis,vis,vis) at every texel even after a camera nudge — the 6.12/6.14 receipts that read `dbg.yzw` through it may have been blind. Ordered next steps: (1) dump the soup's 12 lamp triangles (`soup.tris.value`) and CPU-intersect the segment P→Q for the 5 wall texels at x = 1.43, y 3.9-4.6 — miss ⇒ the tracer is not the culprit; (2) force `pen` to the 6.25 expression to isolate the dilate/filter chain (the one part the spread −1 isolation did not revert); (3) the mover reach on the dynamic arm from the CURRENT seat OBB (W read 2.19 m at every ×2 texel = the 5 % floor engaged); (4) only then the four truth numbers (`scratchpad` rows: ×1 baseline truth ramp 0.858 m / dip 0.29 vs ours 0.562 m / 0.53), `profile.giPasses` (`samples`), two LOOKED-at PNGs (`run-gi2-shot.mjs LAMP_SCALE=`), the default-pose gate. Isolation switches left in: `__giQuadSpread` (0.5 | 0 | −1), `__giVisFromH`, `FLAGS` globals + `ALLCONSOLE=1` + face receipt in `test:gi-scaled-lamp`.

**6.25e (NOT SHIPPED), 2026-08-29, rc5-faces — the tracer is INNOCENT; the texture the test reads is not the raw pass's output.** Receipts (`test:gi-scaled-lamp` on 5206, lamp scale [1,2,1], 74-tri soup, exact arm live): (1) CPU intersection (`FLAGS __cpuHit`, in the test script; brute force over `gi2.shadowBvh.tris` + the JS BVH mirror over the bound `nodes/triIdx`) — the centre segment P→OBB entry and the four quadrant segments at the 5 wall texels x = 1.43, y 3.11-4.33 hit NOTHING except the block at y = 3.11 (tris 0/4, not the lamp; the tree mirror agrees: 1/11/1 vs 0/13/6); the lamp's 12 soup triangles (ids 62-73) are never on a segment. (2) The 6.25 files (`8460bc5` rcDirect+shadowBvh) on the SAME scene/pose today: lit 0.976, the shadowed texel 0.874 (the blurred-umbra problem, min 0.81). HEAD: lit 0.24-0.34, the same texels 0.02 / 0.118 / 0.38 — the regression is real and lives in the two files. (3) Every kernel isolation fails IDENTICALLY (lit 0.26-0.34): `__giQuadSpread −1 + __giVisFromH`, `__giNoQuadFn` (the 6.25 nearest-t call, no wrapper), `__giNoDynQuad` (no dynamic quadrant traces), and BOTH together — a kernel logically equal to 6.25 still reads 0.31. (4) ⭐ THE OWNER'S FINGERPRINT: `__giHitReceipt` writes (quadHits/4, hSum/4, tOcc) into vis channels y/z/w for slot 0 — the readback returns (vis, vis, vis, vis) at every texel, the same (v,v,v,v) the 6.25c debug reads showed. The rawPass CANNOT write four equal channels (inactive slots store 1; the receipt stores 0/0/−1 on a clear texel) and the filter is channel-preserving, so `gi2.rc.resolve.direct.texture` (visA) holds something ANOTHER writer produced (a grey ramp rising toward the ceiling — irradiance-shaped), or the rcDirect whose visA the test reads is not the one being dispatched. The generated WGSL (`__dumpRaw`, `WGSL_OUT=`) is logically correct: every var is declared at function top, `nodeVar12 = 1 − clamp(hSum·0.25)`. NEXT (in order): (a) read visA ONE pass at a time — splice a probe read after rawPass only (before dilate/filter) and compare channels; (b) print identities: `direct.texture === the visA captured by the running rawPass`, the rcDirect build count, and whether `resolve.direct` is rebuilt (mover promotion → lit 1.0 in some runs, 0.45 in others = two instances?); (c) grep every `textureStore` target in rcMerge/gi2System for visA/"rcEmitterVis". Flags left in: `__cpuHit`, `__dumpRaw`, `__giHitReceipt`, `__giNoQuadFn`, `__giNoDynQuad`.
**6.25f SHIPPED 2026-08-29, rc5-faces — the owner was PIPELINE COMPILE LATENCY: the raw pass's first dispatch landed 31 s after the pose, and every read before it sampled a born-zero texture.** The (v,v,v,v) fingerprint of 6.25e was the UN-WRITTEN texture (zeros survive the filter as 0, and `direct.texture` read back through the probe's blending produced the grey ramp), not an alias: the identity receipt (`__rcDirectBuilds/Last` in rcDirect + the test's `ident:` row) shows exactly ONE rcDirect instance per GI2 system (`builds:1`, `texIsLast:true`, `rawInFrame:true`, uuid match) through build, carry and scale — the two-instance hypothesis is REFUTED, and no second writer of `rcEmitterVis` exists. Raw-only hatch (`__giRawOnly`) at settle 5 s read zeros; the SAME pose at 45 s read the correct output (ramp 18 px, medVis shadow 0.5, lit 1) — measured precisely with a first-output wait in the harness: **first dispatch 31.1 s after the pose** (4 slots × 14 inlined trace call sites per slot inflate the WGSL the D3D backend compiles). THE FIX, both by construction: (1) **the kernel call sites collapsed to one per arm** — a WGSL `Loop(4)` with a `select`-chain ray picker (`pickQ`) replaces the JS-unrolled four-quadrant static+dynamic traces in rcDirect; compile fell **31.1 s → 4.9 s** (measured, same scene/pose) and pose-1 output is correct again (ramp 9 px, medVis shadow 0.416, lit 1 ≥ 0.9 under `__giQuadSpread:-1`); (2) **the harness waits for the term's first output** (polls `resolve.direct.penumbra` for any nonzero texel, cached probe) before reading, so a read can never again sample a pre-first-dispatch texture. OPEN: the ×2 pose in the 5.8 m harness room is degenerate (radius 1.73→3.46 m — the exclusion sphere swallows the wall; medVis reads null, movers 0→0 — dynamic-layer admission after a scale not re-checked); re-gate the ramp-widening law on Cornell or scale ≤1.3.
**6.25g receipts, 2026-08-29 16:00, rc5-faces `8ae9488` — 6.25f fix verified live; the size law still needs a non-degenerate pose.** (1) `probe:gi2-cornell` (plain Cornell, no scaling): median |log| **0.132 PASS**, second-difference **0 surfaces PASS** (shadow edges), p90 0.366 (cap 0.35, near-miss), black **6 px** (Box·+Z = the known 6.28 open face), blotch 4 surfaces over 10 % (Green·-Z/Floor·+Z/Mesh·+Z/Box·+Z, the known 10-16 % σ); at rest p50 0.82 %, monotone, 40 frames to 90 %. (2) `test:gi-scaled-lamp SCALE=1.3` (after 6.25f: first output 3.7 s): ×1 ramp 13.5 px / medVis shadow 0.558 / lit 1; ×1.3 medVis shadow 0.732 → the gate's letter PASSES — but the honest reading is the 2.6 m-tall scaled lamp (centre y 4.09, top 5.39) rises past the 3 m block's shadow line: ramp → 0 px, wall depth 0.583 → 0.001, vis ≈ 1 on the whole line. Movers note (one line, per the watch): the scale DID promote the lamp (+1 s: movers 1, voxelsSet 638) and it settled back out by +4 s (movers 0) — the final-gate FAIL is the settle, not a missed promotion. (3) Two LOOKED-at back-wall PNGs (RC5=1, POSE eye −1.2,2.2,2.2 → target 1.0,1.8,−2.5): ×1 shows the Cornell interior with soft GI gradients and the block's shadowed face; ×1.3 shows the same view washed 1.69× brighter (walls clip toward white), block stays dark. VERDICT: the compile-latency fix (6.25f) is proven on every run; the W = L·t_occ/d_b widening law is still UNDEMONSTRATED — the harness room gives no non-degenerate growth pose (×2 swallows the wall, ×1.3 clears the block); next: move the block further from the lamp or scale ≤ 1.15, then re-gate ramp widening.
...
- **6.22 GI Mobility IS the classification** (`rc5-cut`, 2026-08-29 12:25-13:30, user rule: "static = never updating, watch dynamic, auto keeps all static and turns dynamic when moved"): one resolver, `src/modules/gi/window/mobility.js` (`stateOf` → static | dynamic | auto | promoted; `isDynamicNow`; exposed as `gi2.mobility` for the static BVH's mover leaf), read by `#gi2Movers`, the build's watch lists, `slotRegistry.refreshTransforms(isDynamicNow)` (the per-frame audit compares ONLY dynamic-now slots), merging's and shadowMerge's motion watchers (a static member is never released / never a mover), and the fingerprint (a Mobility change is content → rebuild). Static: never walked per frame; its pose is compared only on an announced `transforms` bump and warns ONCE (`mesh X is GI static and moved`). Auto: watched amortised (1/8 per frame; the whole list on a key bump), promoted on first motion → its static slots leave the exact-shadow tree the same frame (6.21's mask; receipt `bvhExcludedCount` first > 0 at frame 1 after the move, PASS ≤ 2) and `requestRebuild("gi2:mobility-promoted")` holds it out of the soup; demoted through 6.21's settle (`GI2_MOVER_SETTLE_FRAMES` 90 still frames → `gi2-mover-settled` rebuild at the new pose), which now calls the resolver's `demote` — one classification, one clock. Receipts (`scripts/run-gi-mobility-probe.mjs`, Cornell): at rest `contentKey.transforms` 0 bumps / 5 s (live Cornell ticked every frame with `gi:slot-moved` — the Ceiling's slot, which the audit no longer compares), cpu 1.49-1.74 ms @ 120 fps, movers 0, `mobility {static 1, auto 6}`; Box→static + 1 m: movers 0, dynVox 0, 1 warn across two moves, soupBuilds still 1; Box→dynamic: dynVox 232 at rest, movers 1 moving; Box→auto: moved → promoted 1 / movers 1 / dynVox 233, +2 s settled → promoted 0 / dynVox 0. Rebased on 23517a0 (6.21/6.21b). ⚠ every promotion/demotion and every static↔dynamic reclass re-runs the soup worker (Cornell 570-615 ms off-thread, main thread 0.1 ms; soupBuilds 1→5 over the (b)-(d) cycle) — GI2 has no per-placement static exclusion, so the soup IS the exclusion. ▶ NOT RUN (time box): Bistro rest/`test:gi-moved-lamp`/`probe:gi2-cornell` parity, `test:gi-dynobj` + `test:gi-lighttree-mover` (old-path suites); the Ceiling's per-frame matrix rewrite (WHO writes it) is still open — it is simply not audited now.

**6.20 the Cornell blotch — spatial scale + first arm (2026-08-29, `rc5-energy`, 35-min box, DIAGNOSIS ONLY, no engine change):**
- Instrument: `MAPS=` now also writes `rows.json` (dense per-pixel x/y/surface/world p/signed log error/noise) so the blotch's SCALE is measured, not eyeballed. Autocorrelation of the signed log error, per surface, in metres (jitter arm, 5205): ACF falls to 0.5 at **0.5-0.6 m on Floor/Green/Mesh/Box·-X, ~0.8 m on Red·+Z** and only reaches 0 at 1.3-1.6 m; in screen px it decays slowly over ~12-16 dump cells (~70-100 px), i.e. it is WORLD-anchored at the c0/c1 probe footprint (spacing0 0.5 m), not a 16-px tile pattern and not lamp-facing-only. Candidates (3)/(4) are out; the owner is (1)/(2): what a probe's bins know.
- Baseline (frozen R2, shipped) at 5205 with the other lane on the GPU: median 0.142, p90 0.627, black 0, blotch σ/mean Green 17.0 / Red 30.1 / Box·-X 20.8 / Floor 14.7 / Mesh 16.5 / Ceiling 13.3 / Box·+Z 15.2 %; second difference passes → the σ is LOW-frequency: err.png shows the 0.3-0.5 m concave-corner bands (white strip ≤0.25 m from the red wall signed log −0.32) and 0.5-1 m blobs on the back wall, not grain.
- `__gi2RcJitter=1` (the paper's R2 walk, one boot): median 0.097, p90 0.512, σ Floor 9.9 / Ceiling 6.5 / Mesh 11.7 / Green 21.2 / Red 27.8 / Box·-X 17.7 %, and **the corner band is gone** (strip ≤0.25 m signed log −0.32 → +0.007); at-rest p90 1.63 → 2.45 %. ⚠ CONFOUNDED: the two boots read DIFFERENT scenes — the reference's emitting area 7.479 vs 6.000 m², [E] hit rate 71.9 vs 26.8 %, E_ref on Green 2.24 vs 1.73 — so `giSystem.state.entries` differs boot to boot (the lamp's transform, a boot-order race) and the reference re-traces from a different room. Until that is pinned, a cross-boot A/B on this gate is not a measurement; only within-boot uniform arms are.
- Term arms within the baseline boot: removing the field (`rcTermField:0`) leaves per-surface residual σ Ceiling 6.5 / Mesh 8.0 / Green 11.6 / Floor 13.0 / Box·-X 17.6 / Red 31.9 %, i.e. the DIRECT term alone already carries Red·+Z's and Box·-X's σ (the lamp's own falloff/penumbra against the reference) while the FIELD owns Ceiling/Mesh/Green/Floor's half. Box·+Z (0.29×) is unchanged by every arm — its darkness is neither term's blotch, it is the mirror block's face reading a field that never saw the lamp's side.
- **6.20b SHIPPED (rc5-energy): the direction index is the ray's RANK within its c0 probe** (`srcDeposit.js` `RC_STRATIFY_BY_RANK`, default ON, `__gi2RcStratify = 0` restores; mirror in `srcRef.js`). [D5] already hands each pixel `probeRayOff + rank`; `rayDirection` read the GLOBAL index, so under the 32-ray cap every probe missed a DIFFERENT third of its 32 bins for ever (offset = scheduler-order sum of the probes claimed before it) and the tile bake's known-bin renormalisation turned that into one brightness bias per probe — the 0.5-0.8 m blotch. Rank-indexed, every probe fires the same canonical run and the truncation is uniform. Confound pinned first: the lamp is the `Light` entity (unit box, Mirror.mat emission 10) — the header now prints each mesh's world matrix/box; two consecutive boots read 6.000 m² at [-0.18,2.83,1.51], so the 7.479 m² boot was the outlier, not the scene file. ⚠ boot-to-boot gain still swings (rank-off boot 0.691×, first baseline 0.929×) — per-surface σ and the strip are the within-boot receipts.
  Same boot family (6.000 m² lamp, 5205, other lane on the GPU), σ/mean per surface Green/Red/Floor/Box·-X/Mesh/Ceiling/Box·+Z, strip = white strip ≤0.25 m from the red wall signed log, rest = at-rest |ΔE|/E p90:
  · rank OFF (baseline)   med 0.371 (gain 0.69) · σ 20.8/30.3/7.8/16.6/14.6/13.6/8.0 · strip −0.266 · rest 1.62 % · black 28
  · jitter only (earlier) med 0.097 · σ 21.2/27.8/9.9/17.7/11.7/6.5/14.9 · strip +0.007 · rest 2.45 %
  · **rank ON (shipped)** med 0.118 · σ 21.2/5.8/9.5/23.2/9.8/5.1/14.8 · strip −0.048 · rest **1.03 %** · black 0
  · rank + jitter         med 0.107 · σ 20.4/5.2/8.8/24.0/9.6/5.3/14.5 · strip −0.027 · rest 4.37 % (jitter buys no σ, costs rest)
  Met: strip ≤ 0.1, rest ≤ 3 %, median ≤ 0.15, black 0. NOT met: σ ≤ 10 % on only 4 of 7 (Green·-Z 21 %, Box·-X 23 %, Box·+Z 15 % — the two mirror-block faces and the wall the block shadows; second difference on Box·-X 0.68 vs ref 0.14 = a sharp structure, the block's own occlusion at the lattice, not the direction set). p90 0.42 still FAIL. ▶ NEXT: the Green/Box residue is the tall block's silhouette at 0.5 m probes (voxel/LOS), a different owner.
- ▶ NEXT (the fix the receipt names, not built): per-PROBE direction stratification — each c0 probe's pixels take R2 indices by their rank within the probe's footprint (a contiguous run per probe, as `srcRays`' header already wants) so every probe's bin set is complete regardless of screen position; and the tile bake's unknown bins reconstructed from the probe's own known octahedral neighbours before falling to the parent. Then re-run the gate twice in ONE boot (jitter is a uniform, `rcJitterX/Y`) so the boot confound cannot vote. Also pin the reference's scene read (the 7.479/6.000 m² lamp).


**6.19-6.28 the third wave (2026-08-29 15:00-19:00; merged to `gi19-stage0` f8934f9):**
- **6.19/b/c/d the RC ray budget** (`rc5-marker`): rays were ONE PER LIT PIXEL (451 k inside Cornell, 339 k hit-shades = `hit#0` 3.08 ms) — the paper's per-c0-probe cap [D1'] was never passed to the GI2 RC path. `RC_TIERS.probeRayCap` 32 (phone 16): GI2 inside 9.4 → 6.9 ms, harness 66 → 120 fps. At-rest |ΔE|/E p90 rose 1.68 → 7.8 (cap 16) / 4.6 % (cap 32) → 6.19d age-aware `BIN_WINDOW = 64` (running mean below N, N-EMA above) + change-reset (parent-cone luma moves > 25 % → weight to N/4): p90 **1.02 %**, frames-to-90 % 52 → 21. ⚠ 6.28 then found the cap's price: with frozen canonical directions every probe misses the same bins forever → **black 34-36 px on Box·+Z** (the block's lamp-facing face at 0.25× truth) — OPEN, see 6.28.
- **6.20 the probe-footprint blotch** (`rc5-energy`): autocorrelation 0.5-0.8 m = probe footprint; the direction index was GLOBAL (`probeRayOff + rank`) so each probe's known-bin set was a fixed function of its screen position → one bias per probe. Rank-within-probe index (`RC_STRATIFY_BY_RANK`): Red σ 30 → 5.8 %, Ceiling 13.6 → 5.1, corner strip −0.27 → −0.05, at-rest p90 1.03 %. Still > 10 % on Green·-Z / Box·-X / Box·+Z (a sharp silhouette structure — the shadow term).
- **6.21/b stale shadow on a moved occluder** (`rc5-faces`): the static BVH never learned a promoted mover left. Per-triangle owner slot + 1024-bit exclusion mask at the leaf, dynamic-only trace merged into rcDirect (nearer t) and the face NEE (`max`), settle → one `gi2-mover-settled` rebuild. Old-shadow visibility 0.42 → 1.0 at 3 s. ⚠ that settle rebuild is what 6.29 fixes (a rebuild restarted the light).
- **6.22 GI Mobility is the classification** (`rc5-cut`, `window/mobility.js`): static never audited/released/re-voxelized (moving one warns once); dynamic watched every frame; auto promotes on a matrix change, demotes after settle. Cornell at rest `contentKey.transforms` 0/5 s (it was the Ceiling's slot re-audited every frame — WHO rewrites its matrix is still open); `profile.gi2.mobility` counts. ⚠ every promotion/settle/reclass re-kicks the soup (Cornell 0.6-0.8 s off-thread, Bistro seconds) — GI2 has no per-placement soup exclusion (next structural unit).
- **6.23 emitter scale** (`rc5-cold`): the seat and dynamic voxels already followed the matrix; the content fingerprint had no scale term so area/Φ/admission stayed at build values. Scale mixed into `#computeFingerprint`; `test:gi-scaled-lamp` 8/8.
- **6.24 the moved-lamp regression** (`rc5-glossy` bisect): 6.18's `If(giNestedView)` on the light node's stack dropped the emitter glow from the main view (Δnew 28 → −0.7); `select` restored (6937636). 6.26 (`rc5-marker` 8c23810, PARKED): the stand-in as a build-time variant via `contextNode.version` — WGSL 7504 → 4101 lines but no measurable ms under contention and the planar nested variant unverified.
- **6.27 the black-pixel bisect** (`rc5-cut`): owner 9e34fff (cap) amplified by 745046d (window); 23517a0/fbe71e7 innocent. Second owner of moved-lamp = every GI2 `build()` clears the cache and re-creates the cascades → the field restarts from black on every promotion/settle/reclass → **6.29** (a rebuild refreshes the transport, never the light).
- **6.28 the rank walk** (`rc5-energy` c40891d, OFF): `nDir = rank + (frame mod K)·rays` covers the bins but **K must be coprime to the 2/4/8 cadence** (K=4 phase-locks c2/c3 → dark votes, gain 0.73; K=3 restores energy, halves black, +55 % frames-to-90 %). Box·+Z stays 0.25× under every arm → not direction coverage; 6.28b instruments that face (direct vs field; at 0.3 m from the lamp most canonical rays hit the lamp's own zeroed voxels).
- ⭐⭐ **rules learned**: a lane's gate under GPU contention is parity only (medians drift, BLACK COUNT is robust); a fresh compute node's first dispatches are dropped while its pipeline compiles (dispatch, wait, dispatch); the harness now forwards `uncapturederror`/`device.lost` (`scripts/lib/webgpuErrorLog.mjs`).

**6.29/b a content rebuild refreshes the transport, never the light (`rc5-cut` 6c22196, c48235b, 6245a7d; 2026-08-29 evening):** every GI2 `build()` cleared the brick cache and re-created the cascades → the field restarted from black on every promotion / settle / reclass / emitter scale (the user's "black flash when a dragged object settles"; `test:gi-moved-lamp` FAIL). `GISystem#rebuild` keeps the live GI2 system when `#gi2Signature` (tier:WxH:ao:emissiveShadows) holds; `gi2System.build()` on a live transport is a CARRY (old soup/voxelizer/dynamic serve until the new soup lands, no `win.reset()`, no `clearPass`; trace/deposit/[J] held until the voxelizer's dirty count drains, Cornell 3-10 frames). 6.29b: the remaining 3-4 black frames were the `GICascadeLight` being disposed and re-added by the compile wave — the light instance is retained and re-pointed. `run-gi2-carry-flash-probe.mjs`: worst per-frame drop −69 % → **−2.7 %, 0 frames over 3 %**; moved-lamp Δnew +27.8; `probe:gi2-param-change` 0 errors (a quality flip still re-allocates). `__gi2CarryState=false` restores.

**6.30 the dark strip in the box/wall gap (`rc5-energy` d058caa; NOT fixed):** Box·+Z (rotated 29°, below the lamp's horizon = purely indirect in truth) reads 0.27× and its E falls MONOTONICALLY toward the convex corner across 8 screen columns (1.05 → 0.048) — a smooth trilinear ramp from bright room-side c0 corners to DARK gap-side corners (x ≈ 2.5, between the box corner 2.45 and the green wall 2.83). Not the ray cap, not placement/LOS (both OFF by default; pixel rays feed every cascade), not the upsample fallback (made same-surface-first: neutral, kept). Mechanism: the gap probes are fed by the strip's own pixel rays whose c0 interval [0, 0.8 m] ends on the shadowed green wall with T = 0, whose hit radiance gathers the SAME dark gap probes → a mutual fixed point near 0; the far field (c1+) never enters those bins. ▶ NEXT: shorten c0's interval to the local gap (paper: interval ≤ nearest-hit distance) or gather the hit's E excluding the pixel's own cell corners; first expose `rcSystem` store/bins/`frame.pixelProbe` for a probe dump.

**6.30b the split, priced (`rc5-energy`; NOT fixed):** the instrument is in — `globalThis.__gi2Rc` is the live `createRcCascades` handle (`store`, `bins`, `frame.pixelProbe`, `resolve`); the probe dump itself was not written. What the reading established, against the paper's fix: (a) "gather E_hit from the coarser cascade" is NOT a line, because `srcTiles` bakes the irradiance atlas for **c0 only** (its header: a merged c0 bin already carries c1-c3, so cascades 1-3 have no tiles) and `gatherAt` reads tiles — an E_hit from c1+ needs a second bake (c1 probes' merged bins → tiles at the c1 spacing) plus a `gatherAt` that keys at `probeSpacing(1, lod)`; (a') the cheap stand-in `gatherAt(hp, hn, null, lodOffset=1)` is UNPOPULATED near the camera — `srcProbes.latticeAt` inserts ONE LOD per Chebyshev distance, so LOD 1 exists at the gap only if the camera once stood far from it (the 6.9 fallback walks coarser shells for the same reason), and a fixed-pose gate has none; (b) "exclude the pixel's own corners" has no pixel to exclude — [J] shades per FACE (`faceSamplePoint`), not per ray. The mutual fixed point also needs a per-hop LOSS to reach 0.048 (with ρ = 1 and a cavity fraction f the fixed point is E = R regardless of f; it is the 0.9ᵏ hop gain compounding through ~10 gap-internal hops), so the c1-tile bake is the construction that removes it: the hit reads a field whose near band is the parent's cone, not its own cell's. ▶ NEXT: bake c1 tiles (one more `bakeTiles` over `bins.cascades[1]`, at the c1 block base) and give `createRcHitShading` a `gatherHit` that reads them; gate Box·+Z E_arm/E_ref ≥ 0.7.

**6.30c the c1 read, built and REFUTED (`rc5-energy`; OFF by default, `__gi2RcHitC1 = 1` arms):** `createSrcTileAtlas` and `createSrcScreenGather` take a `cascade` (the atlas bakes `bins.cascades[c]`'s merged bins with `binGridWidth(c)`, the corners key `packProbeKey(lod, c)` at `probeSpacing(c, lod)`); `rcMerge` builds a second atlas + gather at c1 (`tilesHit`/`gatherHit`, baked after the c0 tiles); `createRcHitShading` reads E_hit from it and falls back to the c0 gather where the c1 corner set is unknown. Paired gate, same build, same GPU contention (five lanes), c1 OFF → ON: **Box·+Z 0.25× → 0.23×**, black 9 → 12 (all Box·+Z), global gain 0.812 → 0.782, median 0.215 → 0.249, at-rest p90 0.94 → 1.13 %, frames-to-90 % 340 → 659; `[J]` WGSL 183 → 280 kB and its pipeline 2.5 → 10.1 s (watchdog re-roll) — the second gather is transcribed whole into the hit kernel. err.png: the strip beside the box's right edge is saturated dark on both arms. So either the hit's E source is not the strip's mechanism, or the c1 read falls back to c0 exactly at those hits (a c1 bin toward ±X across a 0.4 m gap is never fed — its rays end at t < t1 — and the seed-parent fills it from c2, which is the same room the c0 outward bins already carry). ▶ NEXT, in order: (1) the fallback counter in [J] (`STAT_SEC_*` word: hits whose c1 set was unknown) — one number decides between the two readings; (2) the 6.30b probe dump through `__gi2Rc` (gap c0 probes: population, known bins, per-bin L vs the lit end of the face); (3) if the loop is real, the interval cut at the deposit (paper: a c0 ray that ends at t < t1 deposits T = 0 for c0 and its hit's radiance from the c1 lattice as a DIRECTIONAL read, not a hemisphere gather) — not a second gather in [J].

**6.32/b/c/d light accumulates as gradients, never tiles (`rc5-cut` a0d13ae → 0ada622; user 08-29 15:00: "we need a mechanism to eliminate the checkerboard completely — it must accumulate in smooth gradients"):** every bin with a parent is written `(1−m)·parent + m·own` with m = the bin's measured sample maturity; the tile bake admits every bin at full cosine weight and bakes maturity as its coverage, so the 8-corner gather weights each corner by trilinear × maturity — an immature probe beside a mature one is outvoted across the CELL (a gradient), not at the tile edge (a step). The per-tile D3 claim-age term and the 6.1 from-black newborn fade are retired (`__gi2ParentPrior=false` restores). ⭐⭐ two refutations on the way: (b) a full-weight parent imports an IMMATURE parent's darkness (black 11 → 78 at rest) → the parent's own maturity is its confidence, walking up the cascades; (c) still not parity at rest (black 12 → 98): a mature probe's STRUCTURALLY unsampled bins (the folded hemisphere) must stay excluded → (d) the prior applies only while the probe is YOUNG: `p = (1 − min(age/24, 1))·(1−m)·m_par`; past that age an unsampled bin returns before any write (the pre-6.32 exclusion). At rest OFF vs ON same minute: median 0.251 → 0.235, black 64 → 6 (inside the contended census swing) = parity. Mid-yaw shot LOOKED at: street lit in smooth gradients, no tile checkerboard; the tile-edge metric is confounded by content (a frame with no information is "smooth"). Mid-rotation black blocks 6-28 % → 0.1-0.7 %.
**6.34/b/c per-placement exclusion, no soup re-kick (`rc5-cold` 330388b → b960b3d):** the voxelizer reads `soup.triOwner` + the same 1024-bit exclusion mask the shadow BVH honours; `markBoxDirty(min,max)` re-voxelizes only the bricks a placement touches. Promotion = mask bit + dirty its bricks + dynamic layer; settle = append the placement's world triangles as a small soup SEGMENT and re-kick ONLY the shadow BVH worker (Cornell: 86 tris, 3-4 ms wall, mask keeps the old copy hidden until the tree lands), then demote. `#gi2Signature` drops W×H (a resize no longer poisons the carry). Cornell promote+settle: `giRebuilds.runs` +0, `soupBuilds` 1, carry-flash worst −0.2 %; moved-occluder old 0.559 → 1 / new 1 → 0.535 at 0.5 s and settled (the earlier "new = 1" was the probe reading the block's own face — 6.34c moves its camera). Bistro live before: 20 rebuilds in minutes.

**6.31/b the shadow freeze survives a skinned mesh (`rc5-marker` f2757cc, 1a9bccb; user's live Bistro 08-29 15:00: fps 35, CPU 23.3 ms, 645 draws, 9.19 M tris/frame on a 2.83 M scene, `freezeReason: a skinned or morphing mesh is present`):** `ShadowFreezeSystem.update()` returned early the moment any deforming caster existed, handing every CSM cascade back to full per-frame rendering (~500 static casters × cascades × 40 µs). Now: dynamic casters (skinned, morph, `giMobility:"dynamic"`, shadowMerge movers) get `SHADOW_DYNAMIC_LAYER` and leave the static hash; per light a stable key with dynamics present runs `static-cache` (hide dynamics → full render → `copyTextureToTexture` depth → cache) once, then `overlay` per frame (copy cache → map, render ONLY the dynamic layer with autoClear off) via the `ShadowNode.renderShadow` hook; any failure falls back. Harness Bistro (1 cascade): ShadowMap pass 81 draws / 2.86 M tris → **3 / 0.056 M**, frame 613 → 455 draws; overlay vs full-render pixels differing > 64/255: Bistro 0.04 %, Level 0.00 % (LOOKED at). 6.31b: `cacheFits` compared `shadow.mapSize` instead of the depth image size → Level re-rendered + re-allocated 64 MB every frame (fixed). OPEN: the character's OWN shadow not proven in a close-up; the depth copy is a full 4096² blit per cascade; cascade re-pose while the camera moves still re-renders (bounds snapping not implemented); `shadowFreeze`'s per-frame fingerprint now runs with a character present (~1 ms on Bistro).

**6.33/b/c/d the flat / no-colour-bounce verdict (`rc5-glossy` d1cba9a → a3d10e8; user 08-29 15:00 "GI got extremely flat, no brown on the street"):** five-stage colour chain (`probe:gi2-colour-chain`, `probe:gi2-wall-bounce`): (a) material albedo → palette brown ✓ (class #10 [0.227,0.099,0.045] r/b 5.0, KTX2 averages correct); (b) voxel palette at brown faces brown ✓; (c) face cache: `Esun = 0` — but the sun-visibility arm is CORRECT (kernel `sunVis` agrees with CPU rays; the sun direction the cache is handed equals the light's world toTarget; the face NEE uses the voxel arm unconditionally, no BVH dependency): at a 26°-elevation sun the Bistro canyon shadows every in-view face — 0 faces with a clear line to the sun at both poses measured = PHYSICS, not a bug; (d) deposit chroma = albedo chroma by construction; measured ground r/b 1.081 at f = 0.434 brown coverage vs modelled 1.13-1.26 (slightly under, within contention noise); (e) the INDIRECT DEBUG VIEW tone-maps E/π (§19 4.3f) and saturates at bright poses — "flat white" is pose-dependent exposure of the VIEW, not a flat buffer (the view's receipt now prints RAW E p50/p95 vs DISPLAYED E/π). **VERDICT: authoring** — to see directional coloured bounce: raise the sun elevation (or rotate it down the street axis) and cut sky:sun toward 1:1; sky:sun ≈ 3:1 with a shadowed canyon is flat by physics. Harness ticket §6.7 stands (KTX2 transcoder WASM 404s intermittently → clay boots).

**6.35 per-mesh emitter seats; the sky term is the scene’s sky (`rc5-glossy` 02998f2 → 6.35 commit; user 08-29 Bistro: warm blotches on the white building in wrong places, indirect grey):** two seat bugs and one sky bug. (1) `profile.giEmitterSeats` now carries a SOURCE LEDGER per seat (`_emitterSeatMeta`: mesh name, world pos, bounding-sphere radius, raw emissive, fill) — the user’s live table (two seats rgb [0,0,0] radius 20.9 m, two [0,0.017,0] “green ×16674” radius 3.9 m) was untraceable from the uniforms alone. (2) The seat SCORE was `luminance × boundsRadius²`, so a 21 m bounding sphere with fill 6e-5 out-ranked every lamp → dead giants held two of four seats; now the score is the admission ledger’s summed Φ = π·A·L per mesh (`_emitterAdmissionByMesh`). (3) §18.15 damped the seat COLOUR by fill and left the SHAPE the whole mesh’s bounds (a 3.87 m sphere of one bulb’s colour — energy right, distribution wrong = the wrong-place bleed); `consolidateSparse` shrinks the shape by √fill and lifts the colour by 1/fill, so 4πr′²L′ = the mesh’s true Φ on a per-bulb-sized body. (4) `sceneSkyRadiance` read only `environmentIntensity` (the OPEN note: sky chroma unread) → the field’s sky term was grey [1,1,1] under a blue HDRI; now the mean of `scene.environment` (DataTexture path: half-float decode, 4096-sample stride, linear as-is; LDR cube path: 8×8 canvas, sRGB-decoded; cached per texture keyed by `version`; undrawable → the old neutral) × intensity. Receipts (`scripts/run-gi2-seat-audit.mjs`, GAME Bistro, settle 20 s, two runs identical): AFTER seats: slot 0 rgb [109.8,0,0] r 0.354 ← emissive [10,0,0] bsR 1.17 fill 0.091; slots 1-3 rgb [14.5–17.7]³ r 0.08-0.09 ← emissive [10,10,10] bsR 0.11 fill 0.57-0.69 — every seat colour = its mesh’s emissive / fill, no seat rgb ≈ 0, largest radius 0.35 m. BEFORE (gi19-stage0 server, same scene): slot 0 rgb [0,0.017,0] r 3.873 reff 2.26 (green ×16675); slot 1 rgb [0,0.019,0] r 3.873 reff 2.36 (green ×18673); slot 2 rgb [0,0,0] r 20.88; slot 3 rgb [0,0,0] r 20.94 — the user’s live table, reproduced; the two bulb strings (Φ tiny after fill) and the two dead giants no longer out-rank the real lamps, per the ONE admission gate. Sky term: `giSkyTerm.rgb` [0.2006, 0.268, 0.3891] beside `scene.environment` DataTexture intensity 1 (was [1,1,1]). LOOKED-at indirect PNG at the street pose (4 s after the move, GPU shared with a Cornell run): white/stone building neutral grey-white, no warm blotch; a red tint only beside slot 0’s red emissive; shade cool grey; young-field black blocks present at 4 s (6.17/6.32 territory, not this unit). Cornell A/B, both arms under contention (baseline 5202 gi19-stage0 6a85d90 → WIP 5207): median 0.159 → 0.149, p90 0.374 → 0.351, gain 0.867 → 0.876, black 10 → 9 (Box·+Z, the 6.28 open face), blotch 3 → 3, second-difference 0 → 0 = parity; the lamp seat byte-identical (c[-0.10,3.39,1.51] reff 0.89 r 1.73 L[10,10,10], fill 1 so consolidation is a no-op by construction); sky [0,0,0] both (no environment). `probe:gi2-param-change` Cornel: boot errors 0, 8 flips, validation errors 0, first light back after every flip, PASS.
