  ...
  for (const node of nodes) {
    ...
    try {
      ...
      forEachStorageBinding(renderer, node, (binding) => {
        ...
        // `StorageBuffer` (StorageBuffer.js:19) passes `attribute.array`
      ...
      // GUARD IS A MEASURED CRASH FIX. Three's `deleteForCompute`
      // (Bindings.js:157) is
      //
      ...
      //
      // Reachable whenever this sweep evicts a node that was never dispatched:
      // a receipt-only kernel, or a build torn down mid-compile-wave. Measured
      // on a resize churn probe: 49 uncaptured device errors across four
      // resolution hops, 0 after this guard.
      //
...
/**
 * ⭐⭐ THE ONLY PATH TO `GPUBuffer.destroy()` FOR A GI BUFFER.
 *
...
/**
 * The half `detachCpuMirror` cannot reach on its own.
 *
...
export function detachCpuMirror(renderer, attr) {
  ...
  // ── THE SWAP ABOVE FREES NOTHING ON ITS OWN ─────────────────────────────
  //
