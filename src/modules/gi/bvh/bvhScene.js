...
export function buildBvhScene(meshes) {
  ...
  const scene = {
    ...
    /**
     * §19 Stage 0.2b — the five BLAS buffers. On Bistro they are the largest
     * single allocation any GI rebuild makes (positions + indices + UVs over
     * every static triangle), and NOTHING destroyed them before this: the
     * scene object is retired-then-`dispose()`d, and `dispose()` only touched
     * the atlas texture. `GISystem#drainRetiredTargets` reads this list and
     * routes it through `releaseStorageAttributes` on the same 3-frame delay
     * the textures get, for the same "used in a submit" reason.
     */
    ...
    dispose() {
      entries.length = 0;
      // §19 Stage 0.2b — see the `let` note above the five arrays. The GPU
      // buffers themselves are destroyed by the caller's retire queue
      // (`storageAttributes` above); this is the JS side, which no attribute
      // release can reach because THIS SCOPE holds it.
      boundsData = null;
      contentsData = null;
      indexData = null;
      positionData = null;
