...
const cache = new Map(); // path -> { path, material, def, generation, migrated, isVolume, renderable }
...
export async function loadMaterialAsset(path) {
  ...
  if (!entry) {
    ...
    entry = { path, material: new THREE.MeshPhysicalNodeMaterial(), def: { ...MATERIAL_DEFAULTS }, isVolume: false, renderable: true, migrated: false };
...
export function isVolumeMaterial(path) {
  return cache.get(path)?.isVolume === true;
}
...
export function getMaterialInstance(path) {
  return cache.get(path)?.material ?? null;
}
