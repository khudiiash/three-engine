...
function diffNode(expected, live, path, out, { isRoot }) {
  ...
  for (const [type, liveComponent] of liveByType) {
    ...
    if (!expectedComponent) {
      // Physics-derived Colliders are visible/editable live components, but
      // they are regenerated from the mesh and are not a prefab override until
      // the user edits one (ColliderComponent marks autoCustomized on edit).
      if (type === "collider" && liveComponent.props?.autoGenerated && !liveComponent.props?.autoCustomized) continue;
      out.push({ t: path, k: "addComponent", c: type, v: clone(liveComponent.baseProps ?? liveComponent.props) });
      continue;
    }
    // The DESKTOP values: under a platform preview `props` holds a phone
    // config's overrides, which are authored on the def already and must not
    // read as instance overrides of every overridden key.
    const liveProps = normalizeProps(type, liveComponent.baseProps ?? liveComponent.props ?? {});
