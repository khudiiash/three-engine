...
// src/skeleton.hpp), as plain data — no node imports, no THREE — so both the
// retarget script (node) and preview harnesses (browser) can share it.
// `offsets` are rest translations, parent-local, metres; rest local ROTATIONS
// are identity: the animated quats are the complete local rotation.
